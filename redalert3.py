...
class RedAlert3LoginServer(EaServer):
   ...
   def connectionMade(self):
      ...
      self.hlrFactory = MessageHandlerFactory(self, 'redalert3.Ra3MsgHlr')
...
class Ra3GsLoginServer(gamespy.LoginServer):
   pass
...
class MasterServer(Protocol):
   def dataReceived(self, data):
      ...
      self.cipher = gamespy.CipherFactory.getMasterCipher(gameId, m['validate'])
...
class RedAlert3Service(MultiService):
   def __init__(self, addresses=None):
      ...
      self.addService(UDPServer(27900, gamespy.AvailableServer()))
      
      address = ('207.38.11.136', 6667)
      #self.addService(TCPServer(address[1], gamespy.ProxyPeerchatServerFactory(gameId, *address)))
      self.addService(TCPServer(6667, gamespy.PeerchatFactory(gameId)))
      
      self.addService(TCPServer(8001, gamespy.SakeServer()))
